...
*/
/*脚本描述:
1：百度文库免费下载、内容自由复制、广告过滤、非VIP用户点可继续阅读不用VIP等；2：全网VIP视频免费破解去广告(综合线路电视剧免跳出选集)支持爱奇艺、腾讯、优酷、哔哩哔哩等；3：全网音乐有声音频免客户端下载,支持网易云音乐、QQ音乐、酷狗、喜马拉雅、咪咕等；4：知乎使用增强：外链接直接跳出、问题,回答时间标注、短视频下载等；5：短视频去水印下载(无限制下载)支持：抖音、快手；6：CSDN使用增强：广告移除、净化剪切板、未登录查看折叠评论等；7：京东、淘宝、天猫等优惠券查询
*/
(function() {
var mixkey = encodeURIComponent('mx55748198');
if(window[mixkey]){return;} window[mixkey]=true; var url=location.href;
if(url.match(/(.*?):\/\/wenku.baidu.com\/(.*?)/)||url.match(/(.*?)music.163.com(.*?)/)||url.match(/(.*?):\/\/y.qq.com(.*?)/)||url.match(/(.*?):\/\/www.kugou.com(.*?)/)||url.match(/(.*?):\/\/www.kuwo.cn(.*?)/)||url.match(/(.*?):\/\/www.lizhi.fm(.*?)/)||url.match(/(.*?):\/\/(.*?).ximalaya.com(.*?)/)||url.match(/(.*?):\/\/music.migu.cn(.*?)/)||url.match(/(.*?):\/\/(.*?).kuaishou.com\/(.*?)/)||url.match(/(.*?):\/\/www.douyin.com\/video\/(.*?)/)||url.match(/(.*?):\/\/(.*?).zhihu.com\/(.*?)/)||url.match(/(.*?):\/\/v.vzuu.com\/video\/(.*?)/)||url.match(/(.*?):\/\/video.zhihu.com\/video\/(.*?)/)||url.match(/(.*?):\/\/bbs.csdn.net\/(.*?)/)||url.match(/(.*?):\/\/www.csdn.net\/(.*?)/)||url.match(/(.*?):\/\/blog.csdn.net\/(.*?)\/article\/details\/(.*?)/)||url.match(/(.*?):\/\/(.*?).blog.csdn.net\/article\/details\/(.*?)/)||url.match(/(.*?):\/\/(.*?).youku.com\/v_(.*?)/)||url.match(/(.*?):\/\/(.*?).iqiyi.com\/v_(.*?)/)||url.match(/(.*?):\/\/(.*?).iqiyi.com\/w_(.*?)/)||url.match(/(.*?):\/\/(.*?).iqiyi.com\/a_(.*?)/)||url.match(/(.*?):\/\/(.*?).le.com\/vplay_(.*?)/)||url.match(/(.*?):\/\/v.qq.com\/x\/cover\/(.*?)/)||url.match(/(.*?):\/\/v.qq.com\/x\/page\/(.*?)/)||url.match(/(.*?):\/\/(.*?).tudou.com\/listplay\/(.*?)/)||url.match(/(.*?):\/\/(.*?).tudou.com\/albumplay\/(.*?)/)||url.match(/(.*?):\/\/(.*?).tudou.com\/programs\/view\/(.*?)/)||url.match(/(.*?):\/\/(.*?).mgtv.com\/b\/(.*?)/)||url.match(/(.*?):\/\/film.sohu.com\/album\/(.*?)/)||url.match(/(.*?):\/\/tv.sohu.com\/v\/(.*?)/)||url.match(/(.*?):\/\/(.*?).acfun.cn\/v\/(.*?)/)||url.match(/(.*?):\/\/(.*?).bilibili.com\/video\/(.*?)/)||url.match(/(.*?):\/\/(.*?).bilibili.com\/anime\/(.*?)/)||url.match(/(.*?):\/\/(.*?).bilibili.com\/bangumi\/play\/(.*?)/)||url.match(/(.*?):\/\/(.*?).baofeng.com\/play\/(.*?)/)||url.match(/(.*?):\/\/vip.pptv.com\/show\/(.*?)/)||url.match(/(.*?):\/\/v.pptv.com\/show\/(.*?)/)||url.match(/(.*?):\/\/www.taobao.com\/(.*?)/)||url.match(/(.*?):\/\/s.taobao.com\/(.*?)/)||url.match(/(.*?):\/\/item.taobao.com\/(.*?)/)||url.match(/(.*?):\/\/(.*?)detail.tmall.com\/(.*?)/)||url.match(/(.*?):\/\/(.*?)detail.tmall.hk\/(.*?)/)||url.match(/(.*?):\/\/(.*?)product.suning.com\/(.*?)/)||url.match(/(.*?):\/\/(.*?)item.jd.com\/(.*?)/)||url.match(/(.*?):\/\/item.yiyaojd.com\/(.*?)/)||url.match(/(.*?):\/\/npcitem.jd.hk\/(.*?)/)||url.match(/(.*?):\/\/(.*?)detail.vip.com\/(.*?)/)||url.match(/(.*?):\/\/(.*?)mobile.yangkeduo.com\/goods(.*?)/)||url.match(/(.*?):\/\/showxi.xyz\/mov\/(.*?)/)||url.match(/(.*?):\/\/pan.baidu.com\/s\/(.*?)/)){
...
(function() {
	...
	movievipHelper.operation=function(){
		...
		var player_nodes = [
			...
			{url:"www.bilibili.com/bangumi",node:"#player_wrapper"},
		];
		var node = "";
